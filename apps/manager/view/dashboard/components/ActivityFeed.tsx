...
const EVENT_META = {
  SALE:       { icon: ShoppingCartIcon, color: 'bg-emerald-50 text-emerald-600' },
  SHIFT_OPEN: { icon: UserCheckIcon,    color: 'bg-sky-50 text-sky-600' },
  TRANSFER:   { icon: TruckIcon,        color: 'bg-indigo-50 text-indigo-600' },
}
    ...
    <CardContent>
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">На момент завантаження</p>
        </div>
      </div>
      {events.length === 0 ? (
        <p className="text-sm text-slate-400">Подій немає</p>
      ) : (
        <div className="space-y-0 -mx-2">
          {events.map((e, i) => {
            ...
            return (
              <div key={e.id} className="relative flex gap-3 items-start px-2 py-2 rounded-lg hover:bg-slate-50">
                {i < events.length - 1 && (
                ...
                <div className="flex-1 min-w-0 pt-0.5">
                  <div className="text-sm text-slate-900 leading-snug">{e.text}</div>
