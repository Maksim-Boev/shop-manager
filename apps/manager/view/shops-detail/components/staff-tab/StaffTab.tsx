        ...
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {members.map(m => (
              <div
                key={m.userId}
                className="bg-white rounded-xl border border-slate-200 p-5 shadow-[0_1px_2px_rgba(15,23,42,0.04)]"
              >
          ...
          {canManage && (
            <button
              onClick={() => setAssignOpen(true)}
              className="w-full border border-dashed border-slate-300 rounded-xl py-3 text-sm font-semibold text-slate-500 hover:border-indigo-400 hover:text-indigo-600 hover:bg-indigo-50/30 flex items-center justify-center gap-2 transition-colors"
            >
